...
export function SecureInteractionDiagram() {
  return (
    <Card className="p-5 md:p-8" aria-label="Secure interaction architecture diagram">
      <div className="grid gap-5 lg:grid-cols-[0.75fr_1fr_0.75fr] lg:items-center">
        ...
        <div className="border-2 border-primary bg-[#f7f2ff] p-5 text-center">
          <p className="text-xs font-bold uppercase tracking-widest text-primary">Trust Layer</p>
          <div className="mt-4 border-2 border-primary bg-primary p-3 text-primary-foreground">
      ...
      <div className="mt-5 grid gap-3 md:grid-cols-3">
        {["Granular permissions per action", "Immutable ledger of all flows", "Cryptographic proof of execution"].map((item) => (
          <div key={item} className="border-2 border-border bg-background p-4 text-sm font-bold text-muted-foreground">
            {item}
          </div>
        ))}
      </div>
...
export function SystemArchitectureDiagram() {
  return (
    <Card className="p-5 md:p-8" aria-label="System architecture diagram">
      <div className="mx-auto max-w-5xl space-y-4 text-center">
        ...
        <div className="border-2 border-primary bg-[#11111a] p-5 text-white">
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div className="text-left">
              <p className="text-xs font-bold uppercase tracking-widest text-[#d8ff00]">Hyper-Ox Blockchain</p>
              <p className="mt-2 text-2xl font-bold uppercase">Trust, settlement, and audit layer</p>
            </div>
            <p className="text-4xl font-bold text-[#d8ff00]">100%</p>
          </div>
        </div>
      </div>
    </Card>
...
export function LedgerDiagram() {
  return (
    <Card className="p-5 md:p-8" aria-label="Blockchain ledger verification diagram">
      <div className="grid gap-6 lg:grid-cols-[1fr_0.8fr] lg:items-center">
        <div className="border-2 border-border bg-[#11111a] p-6 text-white">
          <div className="flex items-center justify-between">
            <p className="text-sm font-bold uppercase tracking-widest text-[#d8ff00]">Hyper-Ox Ledger</p>
            <span className="border border-[#d8ff00] px-3 py-1 text-xs font-bold uppercase">Live</span>
          </div>
          <div className="mt-8 grid gap-3 md:grid-cols-3">
            {["#4921 verified", "#4922 verifying", "Hash 0x8a..."].map((item, index) => (
              <div key={item} className={`border-2 p-4 ${index === 1 ? "border-[#d8ff00]" : "border-white/20"}`}>
                <FileCheck2 className="mb-4 h-6 w-6 text-[#d8ff00]" />
  ...
  return (
    <div className="border-2 border-border bg-white p-4">
      ...
      <div className="mt-3 flex flex-wrap justify-center gap-2">
        {items.map((item) => (
          <span key={item} className="border border-border px-3 py-2 text-sm font-bold text-foreground">
            {item}
          </span>
        ))}
      </div>
